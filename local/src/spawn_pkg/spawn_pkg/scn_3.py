...
class SpawnNode(Node):
    """
    Spawn node for scenario 3.
    """
    def __init__(self):
        super().__init__("spawn_node")

        # Get the file path for the robot model
        sdf_file_path = os.path.join(get_package_share_directory("spawn_pkg"), "models", "agv_big", "model.sdf")
        ...
        # AGV ids
        agv_ids = ['agv_' + str(i+1) for i in range(6)]
